...
async function enable(id: WALLET_IDS): Promise<WalletMetadata> {
    const wallet = SUPPORTED_WALLETS.find((w: Wallet) => w.getMetadata().id === id);
...
export { enable, getBalance, getNetwork, getUsedAddresses, getUnusedAddresses, getRewardAddresses, getChangeAddress, getUtxos };
